...
{
	(void*)uart_send_msg,(uint8_t*)"void uart_send_msg(uint8_t port,uint8_t* pdata,uint8_t msg_len)",
};
...
uint32_t ascii2hex(uint8_t* ascii, void* hex, uint8_t hex_type, uint16_t ascii_len, uint16_t anasy_method)
{
...
char* skip_mark(char* str, char mark, int8_t step)
{
    while(1)
    {
		if(*str == '\0')
...
//跳到字符串中指定字符的地址
char* find_mark(char* str, char mark, int8_t step)
{
    do{
        if(*str == '\0')
            return NULL;
...
*****************************************************************************/
char* find_marks(char* str, char* marks, int8_t step)
{
    uint8_t i,mark_num;
...
char* get_func_ret_type(char* str, uint8_t* ret_type)
{
	...
	__para_info* ret_info = (__para_info*)ret_type;
	*ret_type = 0;
    //跳过指定符号
    str_soi = skip_mark((char*)str, ' ', 1);
    //查找返回值类型结束位置
    str_eoi = find_marks(str_soi, " *", 1);
	//获取返回类型符号
	memcpy(type, str_soi, str_eoi - str_soi);
    //返回值是否为指针
	str_eoi = skip_mark(str_eoi, ' ', 1);
	if(*str_eoi == '*')
	{
		ret_info->is_point = 1;
		//跳到函数名字位置
...
char* get_func_para(char* str, void* para)
{
    char* str_eoi;
    uint8_t para_type;
    //
...
uint8_t get_func_id(char* func_name, uint8_t name_len)
{
    uint8_t index;
    for(index = 0 ; index < sizeof(usmart_nametab) / sizeof(__func_info) ; index++)
    {
        if(!memcmp(func_name, usmart_nametab[index].name, name_len) && name_len == strlen(usmart_nametab[index].name))
            return index;
    }
    return 0xFF;                    //ERR
}

//将接收到的函数名与本地已经注册的函数进行匹配
char* matching_func_name(char* func_name, uint8_t* matching_index)
{
	uint8_t index,ret_type;
    char *str,*str_tmp;
    ...
    for(index = 0 ; index < sizeof(usmart_nametab) / sizeof(__func_info) ; index++)
    {
        ...
        if(usmart_nametab[index].name == NULL)
        {
			...
			//获取声明中函数返回类型
			str = get_func_ret_type(str, &ret_type);
			//获取函数声明中函数名
...
void get_cmd_func(char* str)
{
	...
	//根据该ID中的函数声明获取函数返回类型
	//get_func_ret_type(char* str, uint8_t* ret_type);
