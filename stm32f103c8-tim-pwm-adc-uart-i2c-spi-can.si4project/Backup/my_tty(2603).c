...
char* skip_mark(char* str, char mark)
{
    char* str_tmp = str;
    while(1)
    {
...
//跳到字符串中指定字符的地址
char* find_mark(char* str, char mark)
{
    do{
        if(*str == '\0')
            return NULL;
...
*****************************************************************************/
char* find_marks(char* str, char* marks)
{
    uint8_t i,mark_num;
...
char* get_func_ret_type(char* str, uint8_t* ret_type)
{
	...
	*ret_type = 0;
    //跳过指定符号
    str_soi = skip_mark((char*)str, ' ');
    //查找返回值类型结束位置
    str_eoi = find_marks(str_soi, " *");
	//获取返回类型符号
	memcpy(type, str_soi, str_eoi - str_soi);
    //返回值是否为指针
	str_eoi = skip_mark(str_eoi, ' ');
	if(*str_eoi == '*')
	{
		ret_info->is_point = 1;
		//跳到函数名字位置
