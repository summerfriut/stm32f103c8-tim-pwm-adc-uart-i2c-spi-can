...
uint8_t get_func_para_num(char* str, uint8_t* para_num)
{
	char *str_soi,*str_eoi;
	uint8_t depth = 0;
	str_soi = find_marks(str_soi, NULL, "(,", 1) + 1;
	while(1)
	{
		str_eoi = get_depth_para(str_soi, NULL, NULL);
		if(!str_eoi)
			return 1;			//ERR
		str_soi = str_eoi + 1;	//指向参数开始位置
		(*para_num)++;
	}
}
...
{
    char *str_soi,*str_eoi;
	uint8_t	matching_index,ret_type;
