...
char* get_func_para_val(char* str, uint8_t str_len, void* para_val, uint8_t* type)
{
	...
	//将ASCII码转换成十进制数字
	if(((*type & 0x7F) == PARA_FLOAT) && find_mark(str, str + str_len, '.', 1))		//十进制表示的float
        *(float*)para_val = str2float(str, str_len, NULL);
	else if(find_str(str, str + str_len, "0x"))										//函数调用中的参数为16进制
	{
		switch(*type & 0x7F)
		{
			case PARA_UINT8_T:
				ascii2hex((uint8_t*)str, para_val, sizeof(uint8_t), 2, 1);
				break;
			case PARA_UINT16_T:
				ascii2hex((uint8_t*)str, para_val, sizeof(uint16_t), 4, 12);
				break;
			case PARA_UINT32_T:
				ascii2hex((uint8_t*)str, para_val, sizeof(uint32_t), 8, 1234);
				break;
			case PARA_FLOAT:
				break;
			case PARA_VOID:
...
void response_func_cmd(char* str)
{
	...
	//获取参数
    get_para(str, func_info[matching_index].declaration, para, para_info, decl_para_num);
}
