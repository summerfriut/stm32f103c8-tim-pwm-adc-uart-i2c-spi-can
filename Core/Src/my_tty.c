...
*****************************************************************************/
void hex_str2hex(char* ascii, void* hex, uint8_t hex_type, int16_t ascii_len, uint16_t anasy_method)
{
    uint8_t i,j,*tmp,*hex_tmp = hex,type_size,type_len,offset;
    if(anasy_method == 0)
    ...
    if((hex_type == PARA_FLOAT) && (ascii_len % 8))
        return;
...
//    }
	...
	for(i = 0 ; 1 ; i++)
    {
        if(ascii_len < type_size * 2)
            type_len = ascii_len;
        else
            type_len = type_size * 2;
        ascii_len -= type_size * 2;
        memset(hex_arr, 0x00, type_size);
        offset = 0x00;
        if(type_len != type_size * 2)
            offset = type_size - (type_len + 1) / 2;
        tmp = hex_tmp + i * type_size;
        ascii2hex_bytes(ascii + i * type_size * 2, &hex_arr[offset], type_len, NULL);
...
void ascii2float(char* ascii, float* flo, uint8_t ascii_len, uint16_t anasy_method)
{
    ...
    for(i = 0 ; i < ascii_len / 8 ; i++)
    {
        if(0 == memcmp(space, ascii + i * 8, 8))
            continue;
        tmp = (uint8_t*)(flo + i);
        hex_str2hex(ascii + i * 8, hex, PARA_UINT8_T, 8, 1);
        for(j = 0 ; j < 4 ; j++)
        {
 ...
 * 输入参数  : char* str      字符串首地址
               char* str_lim  字符串限制地址
               char mark      指定跳过的ascii
               int8_t step    步长
 * 输出参数  : 无
 * 返 回 值  : 
 * 调用关系  : 
...
char* get_func_para_val(char* str, uint8_t str_len, void* para_val, __para_info* info)
{
	...
	if(find_str(str, str + str_len, "0x"))										//函数调用中的参数为16进制
	{
		...
		{
			case PARA_UINT8_T:
				hex_str2hex(str, tmp, type_tmp.type, str_len, 1);
				return tmp + sizeof(uint8_t);
			case PARA_UINT16_T:
				hex_str2hex(str, tmp, type_tmp.type, str_len, 12);
				return tmp + sizeof(uint16_t);
			case PARA_UINT32_T:
				hex_str2hex(str, tmp, type_tmp.type, str_len, 1234);
...
uint8_t matching_func_name(char* func_name, uint8_t* matching_index)
{
    ...
    for(index = 0 ; index < sizeof(func_info) / sizeof(__func_info) ; index++)
    {
		...
		//函数名长度不同，则跳过进行下一匹配
		if(func_info[index].name_len != strlen(func_name))
			continue;
		//函数名匹配判断
		if(0 == memcmp(func_info[index].name, func_name, func_info[index].name_len))
		{
